module.exports = {
	list(callback) {
		var sql = 'SELECT * from dwpt_techsummit.sessoes WHERE ativo=1 ORDER BY dataSessao, horaI';
	...
	read(idsessoes, callback) {
		var sql = "SELECT * from dwpt_techsummit.sessoes where idsessoes=?";	
	...
	valida(data,callback) {
		var sql = "SELECT * FROM dwpt_techsummit.sessoes where dataSessao=? and  ? >= horaI and ? <= horaF or ?  >= horaI and ? <=  horaF or ? < horaI and ? > horaF";	
	...
	createsessoes(data, callback) {
		var sql = "INSERT INTO dwpt_techsummit.sessoes (nome, tituloS, descS, dataSessao, horaI, horaF, ativo) VALUES (?,?,?,?,?,?,1)"; 
	...
	update(idsessoes, data, callback) {
		var sql = "UPDATE dwpt_techsummit.sessoes SET nome=?,dataSessao=?,horaI=?,horaF=? WHERE idsessoes=?"; 
		global.connection.query(
	...
	remove(idsessoes, callback) {
		var sql = "UPDATE dwpt_techsummit.sessoes SET ativo=0 where idSessoes=?"; 
		global.connection.query(sql, [idsessoes], function(error, rows, fields){
			if (error) throw error;
			callback(rows);
		});
